...
/*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*/
bool write_fmtline_bin(FILE *fp, uint32_t current_offset, void *buffer, uint32_t count, int data_size, bool iseof )
{
    BYTE   *p_byte;
    uint16_t *p_word;
    uint32_t *p_dword;
    ...
    switch (data_size)
    {
        ...
        case 2:
            ...
            for (i=0;i<(int)elements;i++)
            {
                ...
                {
                    sprintf(data_str, "%s", binstr((uint64_t)BYTE_SWAP_16(*p_word), bin_string, 2));
                    ++p_word;
                } /* else */
        ...
        case 4:
            ...
            for (i=0;i<(int)elements;i++)
            {
                ...
                if (!big_endian)
                {
                    sprintf(data_str, "%s", binstr((uint64_t)*p_dword++,bin_string,4));
                } /* if (little-endian) */
                else
                {
                    sprintf(data_str, "%s", binstr((uint64_t)BYTE_SWAP_32(*p_dword), bin_string, 4));
                    ++p_dword;
                } /* else */
...
} /* bool write_fmtline_bin(uint32_t current_offset, void *buffer, uint32_t count, uint32_t data_size) */



bool write_fmtline_hex(FILE *fp, uint32_t current_offset, void *buffer, uint32_t count, int data_size, bool iseof )
{
    BYTE   *p_byte;
    uint16_t *p_word;
    uint32_t *p_dword;
    ...
    switch (data_size)
    {
        ...
        case 8:
            ...
            for (i=0;i<(int)elements;i++)
            {
                ...
                if (!big_endian)
                {
                    sprintf(data_str, "%08lX%08lX", (uint32_t)(*p_qword >> 32 ), (uint32_t)(*p_qword & 0xffffffff));
                    ++p_qword;
                } /* if (little-endian) */
                else
                {
                    sprintf(data_str, "%08lX%08lX", (uint32_t)BYTE_SWAP_32((*p_qword & 0xffffffff)), (uint32_t)BYTE_SWAP_32((*p_qword >> 32)) );
                    ++p_qword;
                } /* else */
...
} /* bool write_fmtline_hex(uint32_t current_offset, void *buffer, uint32_t count, uint32_t data_size) */



bool write_fmtline_dec(FILE *fp, uint32_t current_offset, void *buffer, uint32_t count, int data_size, bool iseof )
{
    BYTE   *p_byte;
    uint16_t *p_word;
    uint32_t *p_dword;
...
bool write_fmtline_oct(FILE *fp, uint32_t current_offset, void *buffer, uint32_t count, int data_size, bool iseof )
{
    BYTE   *p_byte;
    uint16_t *p_word;
    uint32_t *p_dword;
...
bool write_fmtline_signed(FILE *fp, uint32_t current_offset, void *buffer, uint32_t count, int data_size, bool iseof )
{
    BYTE   *p_byte;
    uint16_t *p_word;
    uint32_t *p_dword;
    ...
    switch (data_size)
    {
        ...
        case 4:
            ...
            for (i=0;i<(int)elements;i++)
            {
                ...
                {
                    sprintf(data_str, "% 11ld", (int32_t)BYTE_SWAP_32(*(int32_t *)p_dword));
                    ++p_dword;
                } /* else */
...
#else
                    sprintf(data_str, "% 21lld", (int64_t)BYTE_SWAP_64(*(int64_t*)p_qword));
#endif
                    ++p_qword;
                } /* else */
...
bool write_fmtline_float(FILE *fp, uint32_t current_offset, void *buffer, uint32_t count, int data_size, bool iseof )
{
    uint32_t *p_dword;
    uint64_t *p_qword;
    int     i,j;
